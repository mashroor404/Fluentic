...
`modules/Global/Header.module/` matches the Fluentic Webflow navbar with a delayed, continuous scroll-linked squeeze. It stays full-size through the first 60vh, then interpolates over the next 60vh and reaches its final state at 120vh. During that range, the desktop container moves from 77.5rem to 67rem and its top padding from 24px to 16px. At the same time, the Fluentic logo wordmark width, opacity, horizontal scale, and 14px logo gap interpolate to zero while the icon remains visible. The `.is-scrolled` class marks only the completed state. This wordmark behavior is desktop-only. The nav shell is always translucent white with 20px backdrop blur, a 16px radius, 12px padding, and the reference shadow; at 991px and below it switches to a solid white shell and dropdown menu that reveals with a downward translate from the navbar rather than a scale animation.
...
`modules/Contact Us.module/` recreates the reference `/contact-us` page's `<section class="contact-section">` as an editable HubSpot DnD module. It includes the shared eyebrow macro, a split-word heading, repeatable contact methods with editable inline SVG markup and links, an optional testimonial, a native HubSpot `form` selector, and editable privacy copy. The module intentionally does not hardcode or create an account-level form ID; editors select an existing form or create one through the module's Form field in the target portal.

The layout uses the reference's centered heading, white two-column content card, 385px contact column, and 613px light-gray form panel. At tablet sizes the form moves above the contact details, and the layout/padding compresses through the existing 991/767/479px breakpoints. The badge and heading use the established overlapping GSAP reveal, while the contact details, testimonial, and form panel rise into view. GSAP and ScrollTrigger remain supplied by `templates/layouts/base.html`, and reduced-motion users see static content.

The module adds `fh-form-wrap fluentic-contact-form` to the form panel. The final scoped section in `css/elements/_forms.css` uses those classes to style both legacy HubSpot forms and the newer form renderer without changing other theme forms: two-column name rows, white 50px controls, 8px corners, responsive textareas, Fluentic focus/error states, and a full-width primary-style submit button. Native form submit markup cannot be rendered through the button macro, so this is the approved exception to the module button-macro rule.
