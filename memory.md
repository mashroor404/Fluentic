...
`modules/Global/Header.module/` matches the Fluentic Webflow navbar with a continuous scroll-linked squeeze over the first 60% of the viewport height. From scroll progress 0 to 1, the desktop container interpolates from 77.5rem to 67rem and its top padding from 24px to 16px. At the same time, the Fluentic logo wordmark width, opacity, horizontal scale, and 14px logo gap interpolate to zero while the icon remains visible. The `.is-scrolled` class now marks only the completed state at 60vh instead of triggering the animation. This wordmark behavior is desktop-only. The nav shell is always translucent white with 20px backdrop blur, a 16px radius, 12px padding, and the reference shadow; at 991px and below it switches to a solid white shell and dropdown menu that reveals with a downward translate from the navbar rather than a scale animation.
